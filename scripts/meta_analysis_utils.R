# Meta-Analysis Utilities - Functions for performing meta-analysis
library(meta)
library(metafor)

# Validate uploaded data
validate_data <- function(data, validation_level='basic'){
  # Basic column checks
  required_cols <- c('study', 'effect_size', 'se')
...
perform_analysis <- function(heterogeneity_test=TRUE, publication_bias=TRUE, sensitivity_analysis=FALSE, effect_measure='SMD'){
  data <- get('.current_data', envir = get("meta_analysis_env", envir = .GlobalEnv))
...
generate_forest_plot <- function(plot_style='classic', confidence_level=0.95, model='random'){
  ...
  temp_file <- tempfile(pattern = "forest_plot_", fileext = ".png")
...
generate_report <- function(format='html', include_code=FALSE){
  ...
  rmarkdown::render('templates/meta_analysis_report.Rmd',
                    params=list(results=res, include_code=include_code),
